...
tags:
  ...
  - 工作总账
updated: 2026-08-16
status: 当前详细总账（GH5 段已用 2026-08-16 勘误覆盖）
supersedes:
  - 课题整体状况_20260804
...
> [!important] 阅读口径
> 本文主体仍是截至 **2026-08-10** 的详细快照。**GH5 / A1 定量以 2026-08-16 勘误为准**（权威增量：[层A0_27992结果与GH5前置门禁_20260809.md](../04_BSV量子光/层A0_27992结果与GH5前置门禁_20260809.md)）。
>
> **2026-08-16**：GH5-v2（29254）执行/merge PASS，求积 v1=FAIL（永久）。GH5 是 `GH5 estimate / quadrature not closed`，**不是**最终收敛结果；真正 A1 **不能封板**。不要重跑 GH5，不要盲投整套 GH7。
...
$$
\boxed{
\text{经典材料与磁序算符基线基本封板；GH3-v2/GH5-v2 已跑完；}
\quad
\text{求积未关闭，A1 定量不能封板，真正输出光子统计尚未开始。}
}
$$
...
4. **GH5-v2（29254）已跑完，求积未关闭。**执行/merge PASS，`gh3_gh5_quadrature_v1=FAIL`（永久）。GH5 是临时生产估计，不是最终收敛；真正 A1 不能封板。下一步是尾部模型门，不是重跑 GH5，也不是盲投 GH7。
...
| A0 | 复数 \(J_n\)、ICS、CS、共享 \(\pm\mathbf N\) 节点、分块传播 | **GH5-v2 已跑；求积未关闭** | 可讨论 H2 \(\|\bar J_o\|\) 模长（~3%）；完整 Jones/ICS/高阶未收敛 |
| A1 | 跨谐波轨迹矩 \(A_{nm},B_{nm},F_{nm}\) | **可开发，不能封板** | GH5 estimate / quadrature not closed；H2 的 \(A_{22}^{o}/F_{22}^{o}\) 仍变 ~21%/27% |
...
| full112 GH5-v2（29254） | 两态×±N×25 节点；part_1/OMP=36；20 块 | 20/20 + merge PASS；v1 求积 FAIL（永久） | GH3 不够；GH5 未证明收敛；H2 \(\|\bar J_o\|\) 模长 ~3%，复 Jones ~12%；A1 矩未收敛 | 临时生产估计，未封板 |
...
- same-binary / C8↔C36 / GH3-v2：**已关闭**（见 0809 增量记录）；
- 正式 full112 GH5-v2（29254）：**已完成**；求积 v1=FAIL（永久）；标签 `GH5 estimate / quadrature not closed`；
- k50/k60 量子输入生产：**未提交，旧 M88@k50 路线已失效**；
- 真正 A1 定量封板 / B0/B1：**未开始**。GH5 数据可开发初步 A1，但不能当最终收敛。

当前下一步不是再投 GH5，也不是盲投整套 GH7：

1. 零成本诊断（完整 Jones、\(A_{22}^{o}/F_{22}^{o}\)、外圈贡献、CEP‑π 归档）已落地；
2. GH7 前先做最外层反节点尾部模型门；
3. 尾部门通过后再谈 GH5→GH7。
...
**2026-08-16**：v1 门 `FAIL`（永久）。只证明 GH3 不够，**不**证明 GH5 已收敛。v1 的 H2“3%”只是 \(\|\bar J_{o,2}\|\) 模长；完整复 Jones 约 12%。A1 的 \(A_{22}^{o}/F_{22}^{o}\) 约 −21%/−27%。
...
**2026-08-16**：可用 GH5-v2 开发初步矩阵，必须标记 `GH5 estimate / quadrature not closed`。H2 的 \(\bar J_o\) 仍是 A0 一阶平均电流，不是真正 A1。定量结果尚不能封板。
...
3. 两个 full112 GH5 manifests 是否生成、冻结并通过 Q0？**是（已冻结并用于 29254）。**
4. GH3→GH5 时 H2/H5/H7/H9 的 ICS、CS 和复 \(\bar J_o\) 是否收敛？**否。v1=FAIL（永久）。**
5. 贡献节点 k20→k40 是否稳定，H10 是否必须升 k60？**未做；GH7 尾部门优先。**
6. A1 的 \(A_{nm},B_{nm},F_{nm}\) 是否实现并保持 classical-mixture 命名？**可开发，不能封板。**
...
&\text{A0：GH3-v2/GH5-v2 已跑完；求积 v1=FAIL（永久）；}\\
&\text{GH5：临时生产估计，未证明收敛；}\\
&\text{A1：可开发初步矩阵，定量不能封板；}\\
