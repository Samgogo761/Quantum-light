...
from gh3_gh5_quadrature_gate import mixed_rel
...
JONES_PHASE = 0.1
JONES_ETA = 1.0e-4
JONES_ABS_SCALE = 1.0e-3
H10_RTOL = 0.10
GATE_VERSION = "gh7_tail_model_v2_20260816"
IGNORE_ROOT_NAMES = {
    "GH7_TAIL_K20.json",
    "GH7_TAIL_K40.json",
    "job_id.txt",
...
def compare_jones(a: dict, b: dict) -> dict:
    rows = {}
    hard_fail = []
    scale_candidates = [JONES_ABS_SCALE]
    for src in (a, b):
        for jx, jy in src.values():
            scale_candidates.append(jones_amp(jx, jy))
    campaign_scale = max(scale_candidates)
    for order in JONES_HARD + (10,):
        ...
        delta = de["vec_rel"] * max(ref, 1.0e-300)
        mixed = mixed_rel(delta, ref, scale, rtol, JONES_ETA)
        phase_ok = (not mixed["strong"]) or de["phase_rad"] <= JONES_PHASE
        ok = mixed["pass"] and phase_ok
        de["mixed"] = mixed
        de["pass"] = ok
        de["rtol"] = rtol
        de["gate"] = "mixed_abs_rel"
        rows[str(order)] = de
...
def audit_k20_dir(outdir: Path, freeze: dict, probe: dict, nk: int) -> dict:
    ...
    try:
        ...
        pinned_head = freeze.get("git_head")
        if pinned_head and pinned_head != "TO_BE_PINNED" and meta.get("git_head") not in {None, "UNAVAILABLE"}:
            if meta.get("git_head") != pinned_head:
                raise ValueError(f"git_head {meta.get('git_head')} != {pinned_head}")
...
def main() -> int:
    ...
    report: dict = {
        ...
        "thresholds": {
            ...
            "jones_eta": JONES_ETA,
            "jones_abs_scale": JONES_ABS_SCALE,
            "h10_rtol": H10_RTOL,
            "jones_gate": "mixed_abs_rel",
        },
    }
    if args.stage == "k20":
        ...
        if k20 is None or k20.get("status") != "PASS":
            ...
            for name in expected:
                ...
                if not (d20 / "HHG_nodes_modes.dat").is_file():
                    ...
                    continue
                if not (d40 / "HHG_nodes_modes.dat").is_file():
                    pairs.append({"case": name, "status": "INCOMPLETE", "pass": False, "reason": "k40 modes missing"})
                    continue
                cmpj = compare_jones(
                    jones_from_modes(d20 / "HHG_nodes_modes.dat"),
                    jones_from_modes(d40 / "HHG_nodes_modes.dat"),
                )
                pairs.append({"case": name, "k20": str(d20), "status": "PASS" if cmpj["pass"] else "FAIL", **cmpj})
            report["pairs"] = pairs
            if found != expected:
