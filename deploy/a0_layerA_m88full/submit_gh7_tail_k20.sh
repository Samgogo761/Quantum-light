...
PIN_HEAD="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1],encoding="utf-8"))["git_head"])' "${FREEZE}")"
[ "${PIN_HEAD}" != "TO_BE_PINNED" ] || { echo "REFUSED: FREEZE.git_head is not pinned" >&2; exit 1; }
if [ "${HEAD}" != "${PIN_HEAD}" ]; then
  if git -C "${REPO}" diff --quiet "${PIN_HEAD}" HEAD -- . \
      ':!deploy/a0_layerA_m88full/gh7_tail/FREEZE.json' \
      ':!deploy/a0_layerA_m88full/gh7_tail/FREEZE.sha256'; then
    echo "HEAD ${HEAD} is a FREEZE.git_head pin-successor of ${PIN_HEAD}"
  else
    echo "REFUSED: git HEAD ${HEAD} != FREEZE.git_head ${PIN_HEAD}" >&2
    exit 1
  fi
fi
...
cd "${RUNDIR}"
unset WORKDIR || true
JOBID="$(sbatch --parsable --chdir="${RUNDIR}" --export=ALL,OUTROOT="${OUTROOT}",REPO="${REPO}",NK=20,GH7_TAIL_SUBMIT="${GH7_TAIL_SUBMIT}" "${SCRIPT}")"
