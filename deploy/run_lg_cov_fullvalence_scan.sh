#!/bin/bash
#=============================================================================
# LG-covariant full-valence / conduction-cut scan for CrI3.
#
# Purpose:
#   Keep all 84 occupied valence bands and truncate only high conduction bands.
#   This tests whether an asymmetric window is a cheaper alternative to
#   full112 for classical-light benchmarks or later BSV approximations.
#
# Recommended no-dephasing benchmark:
#   sbatch --array=0-2 deploy/run_lg_cov_fullvalence_scan.sh
#
# Recommended selected-T2 rerun after T2 is chosen, for example:
...
#SBATCH --output=lgcov_fval_%A_%a.out
#SBATCH --error=lgcov_fval_%A_%a.err
...
OUTROOT="${OUTROOT:-${WORKDIR}/output_lg_cov_fullvalence_nodeph}"
T2_FS="${T2_FS:-1.0e30}"
T2_CYCLES="${T2_CYCLES:--1.0}"
DT_AU="${DT_AU:-0.35}"

CASES=(
  "lgcov_k40_nb94 40 1 94"
  "lgcov_k40_nb104 40 1 104"
  "lgcov_k40_nb112 40 1 112"
)
...
echo "============================================="
echo "  LG-covariant full-valence scan case"
echo "============================================="
echo "  Date:       $(date)"
echo "  Host:       $(hostname)"
...
compile_if_needed() {
  ...
  local lockdir="${WORKDIR}/.compile_lg_cov_fullval.lockdir"
